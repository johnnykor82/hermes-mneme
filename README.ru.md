...
Версия 0.4.0 использует нативные хуки Hermes `select_context()` и
`on_turn_complete()`, если доступны оба. Подбор контекста меняет только запрос
к модели, а завершённый ход сохраняется в памяти без принудительного вызова
`compress()` на каждом шаге. На старых сборках Hermes сохраняется прежний
режим через компрессию. После обновления плагина перезапустите Hermes.
